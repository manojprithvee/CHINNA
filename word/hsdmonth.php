...
$query_Recordset1 = "SELECT monthname(date) AS month,year(date) AS year,sum(amount) AS amount,sum(sales) AS sales,sum(`recipt no`) AS recpitno FROM hsd GROUP BY month(date),year(date);";
$Recordset1 = mysql_query($query_Recordset1, $database) or die(mysql_error());
$row_Recordset1 = mysql_fetch_assoc($Recordset1);
$totalRows_Recordset1 = mysql_num_rows($Recordset1);

	header("Content-type: application/vnd.ms-word");
	header("Content-Disposition: attachment; Filename=hsdmonth.doc");
?>
